...
import {doPosting} from "../App/solid";
...
const PostModel = (props) => {
  ...
  const postArticleHandler = async (e) => {
    if (e.target === e.currentTarget) {
      if (image || vedio) {
        props.uploadPost({ image, vedio, text });
      } else {
       if(session.info.isLoggedIn){
        const post = document.getElementById("description").value;
        const webId = session.info.webId;
        await doPosting(webId,post);
       }
      reset();
      props.close();
    }}
  ...
  return (
    <Container>
      <input type="hidden" value="" id="MyPostUrl" />
      <Content>
        <Header>
          <h2>Create a post</h2>
          <button
        ...
        <SharedContent>
          <UserInfo>
            <img src= {props ? props.photo : "/Images/user.svg"}
              alt="user"
            />
